    ...
    )
{
    ...
    if (openPrinterDefaults)
    {
        SecurityHelper::DemandPrintingPermissions(openPrinterDefaults->DesiredAccess);
    }
    ...
    )
{
    ...
    if (!win32PrinterInfo->IsInvalid)
    {
        if (level != 9)
        {
            SecurityHelper::DemandAllPrinting();
        }
...
///     TreatAsSafe -   Demands printing permissions
/// </SecurityNote>
IPrinterInfo^
PrinterThunkHandler::
GetManagedPrinterInfoObject(
    ...
    )
{
    if (isInPartialTrust)
    {
        (gcnew SecurityPermission(SecurityPermissionFlag::UnmanagedCode))->Assert();
    }
...
ThunkIsMetroDriverEnabled()
{
    ...
    if (byteCount > 0)
    {
        if (SafeMemoryHandle::TryCreate(byteCount, win32HeapBuffer))
        {
            ...
            if (returnValue)
            {
                (gcnew SecurityPermission(SecurityPermissionFlag::UnmanagedCode))->Assert(); //BlessedAssert
...
/// TreatAsSafe - Handle will not be freed by SafeMemoryHandle instance.
///               The memory handle is still treated as a protected resource. For example, memory read/write is allowed only through critical methods.
///</SecurityNote>
SafeMemoryHandle^
SafeMemoryHandle::
Wrap(
...
///             - Cannot annotate the method with SecurityCritical because the base class does not
///               annotate its ReleaseHandle with SecurityCritical
///</SecurityNote>
Boolean
SafeMemoryHandle::
ReleaseHandle(
...
/*--------------------------------------------------------------------------------------*/
/*                    SecurityHelper Implementation                                   */
/*--------------------------------------------------------------------------------------*/

void
MS::Internal::PrintWin32Thunk::SecurityHelper::
DemandAllPrinting(
    void
    )
{
    PrintingPermission^  permission = gcnew PrintingPermission(PrintingPermissionLevel::AllPrinting);
...
{
    PrintingPermission^  permission = gcnew PrintingPermission(PrintingPermissionLevel::DefaultPrinting);
    ...
    )
{
    ...
    {
        case PrintSystemDesiredAccess::AdministrateServer:
        case PrintSystemDesiredAccess::AdministratePrinter:
        {
            MS::Internal::PrintWin32Thunk::SecurityHelper::DemandAllPrinting();
            break;
        }
        case PrintSystemDesiredAccess::EnumerateServer:
        case PrintSystemDesiredAccess::UsePrinter:
        case PrintSystemDesiredAccess::None:
        default:
        {
            break;
        }
