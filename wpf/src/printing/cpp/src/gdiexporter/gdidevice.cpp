...
bool HasUnmanagedCodePermission(void)
{
    try
    {
        SecurityPermission^ permit = gcnew SecurityPermission(SecurityPermissionFlag::UnmanagedCode);
...
{
 #ifdef DBG
    if (Microsoft::Internal::AlphaFlattener::Utility::DisplayPageDebugHeader && HasUnmanagedCodePermission())
    {
        FinePrint(
            m_hDC->GetHDC(),
