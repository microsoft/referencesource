...
Stream^ FontStreamContext::GetStream()
{
    if (_stream == nullptr)
    {
        if (_sourceUri != nullptr && _sourceUri->IsFile)
        {
            ...
            FileIOPermission^ fileIOPermission = gcnew FileIOPermission(FileIOPermissionAccess::Read, _sourceUri->LocalPath);
            fileIOPermission->Assert(); // BlessedAssert
        ...
        {
            // open stream using GlyphTypeface
            CodeAccessPermission^ fontReadPermission = _sourceTypeface->CriticalFileReadPermission;

            if (fontReadPermission != nullptr)
            {
                fontReadPermission->Assert(); // Blessedassert
            }
            ...
            finally
            {
                if (fontReadPermission != nullptr)
                {
                    fontReadPermission->RevertAssert();
                }
            }
