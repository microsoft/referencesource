//---------------------------------------------------------------------------
//
// <copyright file="SizeAnimationUsingKeyFrames.cs" company="Microsoft">
...
{
    /// <summary>
    /// This class is used to animate a Size property value along a set
    /// of key frames.
    /// </summary>
    ...
    public class SizeAnimationUsingKeyFrames : SizeAnimationBase, IKeyFrameAnimation, IAddChild
    {
        ...
        /// <Summary>
        /// Creates a new KeyFrameSizeAnimation.
        /// </Summary>
        ...
        {    
            ...
            if (sourceAnimation._keyFrames != null)
            {
                ...
                {
                    _keyFrames = (SizeKeyFrameCollection)sourceAnimation._keyFrames.CloneCurrentValue();
                }
                else
                {
                    _keyFrames = (SizeKeyFrameCollection)sourceAnimation._keyFrames.Clone();
                }
        ...
        /// </returns>
        protected sealed override Size GetCurrentValueCore(
            Size defaultOriginValue,
            Size defaultDestinationValue, 
            AnimationClock animationClock)
        {
            ...
            else
            {
                ...
                if (currentResolvedKeyFrameIndex == 0)
                {
                    ...
                    if (IsAdditive)
                    {
                        fromValue = AnimatedTypeHelpers.GetZeroValueSize(defaultOriginValue);
                    }
                    else
                    {
            ...
            if (IsCumulative)
            {
                ...
                if (currentRepeat > 0.0)
                {
                    currentIterationValue = AnimatedTypeHelpers.AddSize(
                        ...
                        AnimatedTypeHelpers.ScaleSize(GetResolvedKeyFrameValue(maxKeyFrameIndex), currentRepeat));
                }
            }
            ...
            if (IsAdditive)
            {                
                return AnimatedTypeHelpers.AddSize(defaultOriginValue, currentIterationValue);
            }
        ...
        /// <summary>
        /// Returns the SizeKeyFrameCollection used by this KeyFrameSizeAnimation.
        /// </summary>
        IList IKeyFrameAnimation.KeyFrames
        {
            ...
            set
            {
                KeyFrames = (SizeKeyFrameCollection)value;
            }
        }
        ...
        public SizeKeyFrameCollection KeyFrames
        {
            get
            {
                ...
                if (_keyFrames == null)
                {
                    if (this.IsFrozen)
                    {
                        _keyFrames = SizeKeyFrameCollection.Empty;
                    }
                    else
                    {
                        WritePreamble();

                        _keyFrames = new SizeKeyFrameCollection();
        ...
        /// 0.0 there is slightly different behavior between KeyFrameSizeAnimations
        ...
        private Size GetResolvedKeyFrameValue(Int32 resolvedKeyFrameIndex)
        {
        ...
        private SizeKeyFrame GetResolvedKeyFrame(Int32 resolvedKeyFrameIndex)
        {
        ...
        {
            Debug.Assert(!_areKeyTimesValid, "KeyFrameSizeAnimaton.ResolveKeyTimes() shouldn't be called if the key times are already valid.");
        ...
        private void ResolvePacedKeyTimes()
        {
            ...
            do
            {
                if (_keyFrames[index].KeyTime.Type == KeyTimeType.Paced)
                {
                    ...
                    Size prevKeyValue = _keyFrames[index - 1].Value;

                    do
                    {
                        Size currentKeyValue = _keyFrames[index].Value;

                        // Determine the segment length for this key frame and
                        // add to the total length.
                        totalLength += AnimatedTypeHelpers.GetSegmentLengthSize(prevKeyValue, currentKeyValue);
                    ...
                    totalLength += AnimatedTypeHelpers.GetSegmentLengthSize(prevKeyValue, _keyFrames[index].Value);
