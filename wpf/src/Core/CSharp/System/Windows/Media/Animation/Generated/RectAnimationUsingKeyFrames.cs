//---------------------------------------------------------------------------
//
// <copyright file="RectAnimationUsingKeyFrames.cs" company="Microsoft">
...
{
    /// <summary>
    /// This class is used to animate a Rect property value along a set
    /// of key frames.
    /// </summary>
    ...
    public class RectAnimationUsingKeyFrames : RectAnimationBase, IKeyFrameAnimation, IAddChild
    {
        ...
        /// <Summary>
        /// Creates a new KeyFrameRectAnimation.
        /// </Summary>
        ...
        /// </returns>
        protected sealed override Rect GetCurrentValueCore(
            Rect defaultOriginValue,
            Rect defaultDestinationValue, 
            AnimationClock animationClock)
        {
            ...
            else
            {
                ...
                if (currentResolvedKeyFrameIndex == 0)
                {
                    ...
                    if (IsAdditive)
                    {
                        fromValue = AnimatedTypeHelpers.GetZeroValueRect(defaultOriginValue);
                    }
                    else
                    {
            ...
            if (IsCumulative)
            {
                ...
                if (currentRepeat > 0.0)
                {
                    currentIterationValue = AnimatedTypeHelpers.AddRect(
                        ...
                        AnimatedTypeHelpers.ScaleRect(GetResolvedKeyFrameValue(maxKeyFrameIndex), currentRepeat));
                }
            }
            ...
            if (IsAdditive)
            {                
                return AnimatedTypeHelpers.AddRect(defaultOriginValue, currentIterationValue);
            }
        ...
        /// <summary>
        /// Returns the RectKeyFrameCollection used by this KeyFrameRectAnimation.
        /// </summary>
        IList IKeyFrameAnimation.KeyFrames
        {
            ...
            set
            {
                KeyFrames = (RectKeyFrameCollection)value;
            }
        }
        ...
        public RectKeyFrameCollection KeyFrames
        {
            get
            {
                ...
                if (_keyFrames == null)
                {
                    if (this.IsFrozen)
                    {
                        _keyFrames = RectKeyFrameCollection.Empty;
                    }
                    else
                    {
                        WritePreamble();

                        _keyFrames = new RectKeyFrameCollection();
        ...
        /// 0.0 there is slightly different behavior between KeyFrameRectAnimations
        ...
        private Rect GetResolvedKeyFrameValue(Int32 resolvedKeyFrameIndex)
        {
        ...
        private RectKeyFrame GetResolvedKeyFrame(Int32 resolvedKeyFrameIndex)
        {
        ...
        {
            Debug.Assert(!_areKeyTimesValid, "KeyFrameRectAnimaton.ResolveKeyTimes() shouldn't be called if the key times are already valid.");
        ...
        private void ResolvePacedKeyTimes()
        {
            ...
            do
            {
                if (_keyFrames[index].KeyTime.Type == KeyTimeType.Paced)
                {
                    ...
                    Rect prevKeyValue = _keyFrames[index - 1].Value;

                    do
                    {
                        Rect currentKeyValue = _keyFrames[index].Value;

                        // Determine the segment length for this key frame and
                        // add to the total length.
                        totalLength += AnimatedTypeHelpers.GetSegmentLengthRect(prevKeyValue, currentKeyValue);
                    ...
                    totalLength += AnimatedTypeHelpers.GetSegmentLengthRect(prevKeyValue, _keyFrames[index].Value);
