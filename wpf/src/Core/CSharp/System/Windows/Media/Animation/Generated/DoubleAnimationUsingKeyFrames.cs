//---------------------------------------------------------------------------
//
// <copyright file="DoubleAnimationUsingKeyFrames.cs" company="Microsoft">
...
namespace System.Windows.Media.Animation
{
    ...
    public class DoubleAnimationUsingKeyFrames : DoubleAnimationBase, IKeyFrameAnimation, IAddChild
    {
        ...
        /// </returns>
        protected sealed override Double GetCurrentValueCore(
            ...
            AnimationClock animationClock)
        {
            ...
            else
            {
                ...
                if (currentResolvedKeyFrameIndex == 0)
                {
                    ...
                    if (IsAdditive)
                    {
                        fromValue = AnimatedTypeHelpers.GetZeroValueDouble(defaultOriginValue);
                    }
                    else
                    {
            ...
            if (IsCumulative)
            {
                ...
                {
                    currentIterationValue = AnimatedTypeHelpers.AddDouble(
                        currentIterationValue,
                        AnimatedTypeHelpers.ScaleDouble(GetResolvedKeyFrameValue(maxKeyFrameIndex), currentRepeat));
                }
            }
            ...
            if (IsAdditive)
            {                
                return AnimatedTypeHelpers.AddDouble(defaultOriginValue, currentIterationValue);
            }
        ...
        /// <summary>
        /// Returns the DoubleKeyFrameCollection used by this KeyFrameDoubleAnimation.
        /// </summary>
        IList IKeyFrameAnimation.KeyFrames
        {
            ...
            set
            {
                KeyFrames = (DoubleKeyFrameCollection)value;
            }
        }
        ...
        public DoubleKeyFrameCollection KeyFrames
        {
            get
            {
                ...
                if (_keyFrames == null)
                {
                    if (this.IsFrozen)
                    {
                        _keyFrames = DoubleKeyFrameCollection.Empty;
                    }
                    else
                    {
                        WritePreamble();
        ...
        /// 0.0 there is slightly different behavior between KeyFrameDoubleAnimations
        ...
        private Double GetResolvedKeyFrameValue(Int32 resolvedKeyFrameIndex)
        {
        ...
        private DoubleKeyFrame GetResolvedKeyFrame(Int32 resolvedKeyFrameIndex)
        {
        ...
        {
            Debug.Assert(!_areKeyTimesValid, "KeyFrameDoubleAnimaton.ResolveKeyTimes() shouldn't be called if the key times are already valid.");
        ...
        private void ResolvePacedKeyTimes()
        {
            ...
            do
            {
                if (_keyFrames[index].KeyTime.Type == KeyTimeType.Paced)
                {
                    ...
                    Double prevKeyValue = _keyFrames[index - 1].Value;

                    do
                    {
                        Double currentKeyValue = _keyFrames[index].Value;

                        // Determine the segment length for this key frame and
                        // add to the total length.
                        totalLength += AnimatedTypeHelpers.GetSegmentLengthDouble(prevKeyValue, currentKeyValue);
                    ...
                    totalLength += AnimatedTypeHelpers.GetSegmentLengthDouble(prevKeyValue, _keyFrames[index].Value);
