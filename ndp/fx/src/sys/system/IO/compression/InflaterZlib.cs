...
namespace System.IO.Compression
{
    ...
    internal class InflaterZlib : IInflater
    {
        ...
        private void InflateInit(int windowBits)
        {
            ...
            {
                throw new ZLibException(SR.GetString(SR.ZLibErrorDLLLoadError), exception);
            }
            ...
            switch (error)
            {
                ...
                case ZLibNative.ErrorCode.MemError:     // Not enough memory
                    throw new ZLibException(SR.GetString(SR.ZLibErrorNotEnoughMemory), "inflateInit2_", (int)error, _zlibStream.GetErrorMessage());

                case ZLibNative.ErrorCode.VersionError: //zlib library is incompatible with the version assumed
                    throw new ZLibException(SR.GetString(SR.ZLibErrorVersionMismatch), "inflateInit2_", (int)error, _zlibStream.GetErrorMessage());

                case ZLibNative.ErrorCode.StreamError:  // Parameters are invalid
                    throw new ZLibException(SR.GetString(SR.ZLibErrorIncorrectInitParameters), "inflateInit2_", (int)error, _zlibStream.GetErrorMessage());

                default:
                    throw new ZLibException(SR.GetString(SR.ZLibErrorUnexpected), "inflateInit2_", (int)error, _zlibStream.GetErrorMessage());
            }
        }
        ...
        private ZLibNative.ErrorCode Inflate(ZLibNative.FlushCode flushCode)
        {
            ...
            switch (errC)
            {
                ...
                case ZLibNative.ErrorCode.MemError:     // Not enough memory to complete the operation
                    throw new ZLibException(SR.GetString(SR.ZLibErrorNotEnoughMemory), "inflate_", (int)errC, _zlibStream.GetErrorMessage());
                ...
                case ZLibNative.ErrorCode.StreamError:  //the stream structure was inconsistent (for example if next_in or next_out was NULL),
                    throw new ZLibException(SR.GetString(SR.ZLibErrorInconsistentStream), "inflate_", (int)errC, _zlibStream.GetErrorMessage());

                default:
                    throw new ZLibException(SR.GetString(SR.ZLibErrorUnexpected), "inflate_", (int)errC, _zlibStream.GetErrorMessage());
            }
        }
