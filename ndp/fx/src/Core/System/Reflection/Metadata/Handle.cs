...
namespace System.Reflection.Metadata
{
    ...
    internal struct Handle : IEquatable<Handle>
    {
        ...
        internal Handle(byte vType, int value)
        {
            ...
            Debug.Assert(value >= 0, "value should not be negative");
            ...
            Debug.Assert(IsHeapHandle && value <= HeapHandleType.OffsetMask || 
                            !IsHeapHandle && value <= TokenTypeIds.RIDMask, "invalid value for heap handle");
        }
        ...
        internal int RowId
        {
            get
            {
                Debug.Assert(!IsHeapHandle, "should not be a heap handle");
                return _value;
            }
        }
        ...
        internal int Offset
        {
            get
            {
                Debug.Assert(IsHeapHandle, "should be a heap handle");
                return _value;
            }
        }
        ...
        internal int Token
        {
            get
            {
                Debug.Assert(IsEntityOrUserStringHandle, "should be entity or user string handle");
                Debug.Assert(!IsVirtual, "should not be virtual");
                Debug.Assert((_value & TokenTypeIds.RIDMask) == _value, "value should be RIDMask");
