    ...
    public sealed class CertificateRequest
    {
        ...
        public CertificateRequest(string subjectName, ECDsa key, HashAlgorithmName hashAlgorithm)
        {
            ...
            if (string.IsNullOrEmpty(hashAlgorithm.Name))
                throw new ArgumentException(SR.GetString(SR.Cryptography_HashAlgorithmNameNullOrEmpty), nameof(hashAlgorithm));
        ...
        {
            if (_generator == null)
                throw new InvalidOperationException(SR.GetString(SR.Cryptography_CertReq_NoKeyProvided));
        ...
        public X509Certificate2 CreateSelfSigned(DateTimeOffset notBefore, DateTimeOffset notAfter)
        {
            if (notAfter < notBefore)
                throw new ArgumentException(SR.GetString(SR.Cryptography_CertReq_DatesReversed));
            if (_key == null)
                throw new InvalidOperationException(SR.GetString(SR.Cryptography_CertReq_NoKeyProvided));
            ...
            byte[] serialNumber)
        {
            ...
            if (!issuerCertificate.HasPrivateKey)
                throw new ArgumentException(SR.GetString(SR.Cryptography_CertReq_IssuerRequiresPrivateKey), nameof(issuerCertificate));
            if (notAfter < notBefore)
                throw new ArgumentException(SR.GetString(SR.Cryptography_CertReq_DatesReversed));
            if (serialNumber == null || serialNumber.Length < 1)
                throw new ArgumentException(SR.GetString(SR.Arg_EmptyOrNullArray), nameof(serialNumber));
            ...
            if (basicConstraints == null)
                throw new ArgumentException(SR.GetString(SR.Cryptography_CertReq_BasicConstraintsRequired), nameof(issuerCertificate));
            if (!basicConstraints.CertificateAuthority)
                throw new ArgumentException(SR.GetString(SR.Cryptography_CertReq_IssuerBasicConstraintsInvalid), nameof(issuerCertificate));
            if (keyUsage != null && (keyUsage.KeyUsages & X509KeyUsageFlags.KeyCertSign) == 0)
                throw new ArgumentException(SR.GetString(SR.Cryptography_CertReq_IssuerKeyUsageInvalid), nameof(issuerCertificate));
            ...
            try
            {
                switch (keyAlgorithm)
                {
                    case Oids.RsaRsa:
                        if (_rsaPadding == null)
                        {
                            throw new InvalidOperationException(SR.GetString(SR.Cryptography_CertReq_RSAPaddingRequired));
                        }
                    ...
                    default:
                        throw new ArgumentException(
                            SR.GetString(SR.Cryptography_UnknownKeyAlgorithm, keyAlgorithm),
                            nameof(issuerCertificate));
                }
            ...
            byte[] serialNumber)
        {
            ...
            Collection<X509Extension> tbsExtensions = tbsCertificate.Extensions;

            foreach (X509Extension extension in CertificateExtensions)
            {
                tbsExtensions.Add(extension);
            }
