    ...
    public partial struct ECCurve
    {
        ...
        public Oid Oid
        {
            get
            {
                ...
                return _oid == null ? null : new Oid(_oid.Value, _oid.FriendlyName);
            }
            private set
            {
                if (value == null)
                ...
                if (string.IsNullOrEmpty(value.Value) && string.IsNullOrEmpty(value.FriendlyName))
                    throw new ArgumentException(SR.GetString(SR.Cryptography_InvalidCurveOid));
        ...
        public void Validate()
        {
            if (IsNamed)
            {
                ...
                {
                    throw new CryptographicException(SR.GetString(SR.Cryptography_InvalidECNamedCurve));
                }

                if (Oid == null ||
                    (string.IsNullOrEmpty(Oid.FriendlyName) && string.IsNullOrEmpty(Oid.Value)))
                {
                    throw new CryptographicException(SR.GetString(SR.Cryptography_InvalidCurveOid));
                }
            }
            else if (IsExplicit)
            {
                ...
                if (IsPrime)
                {
                    ...
                    if (hasErrors)
                        throw new CryptographicException(SR.GetString(SR.Cryptography_InvalidECPrimeCurve));
                }
                else if (IsCharacteristic2)
                {
                    ...
                    if (hasErrors)
                        throw new CryptographicException(SR.GetString(SR.Cryptography_InvalidECCharacteristic2Curve));
                }
            }
            else
            {
                // Implicit; if there are any values, throw
                System.Diagnostics.Debug.Assert(CurveType == ECCurveType.Implicit);
                if (HasAnyExplicitParameters() || Oid != null)
                {
                    throw new CryptographicException(SR.GetString(SR.Cryptography_CurveNotSupported, CurveType.ToString()));
                }
            }
        }
