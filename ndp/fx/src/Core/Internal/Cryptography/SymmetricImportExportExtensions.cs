            ...
            using (MemoryStream ms = new MemoryStream(keyBlob))
            {
                using (BinaryReader br = new BinaryReader(ms, Encoding.Unicode))
                {
                    ...
                    if (cbSize != SizeOf_NCRYPT_KEY_BLOB_HEADER_SIZE)
                        throw new CryptographicException(SR.GetString(SR.Cryptography_KeyBlobParsingError));
                    ...
                    if (algorithmName != algorithm)
                        throw new CryptographicException(SR.GetString(SR.Cryptography_CngKeyWrongAlgorithm, algorithmName, algorithm));
