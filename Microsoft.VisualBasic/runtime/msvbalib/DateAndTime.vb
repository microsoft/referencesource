...
#Else 'We don't allow you to set the date on Telesto
...
#Else
        ReadOnly Property TimeOfDay() As DateTime
#End If
            Get
...
#If Not TELESTO Then
            'This function is apparently trying to determine a different default for East Asian systems. Why only East Asia
            'would get a separate default is a mystery.  Vb6 compatability?  SystemMaxDBCSCharSize is not available on Telesto.  Telesto
            'doesn't have ANSI code page data, and since Telesto runs on the MAC it wouldn't be available there, anyway.  

            'The international PMs say that this is a broken way of determining what action to take for formatting, etc.
...
#If TELESTO Then
                Interval = Interval.ToUpper(CultureInfo.InvariantCulture) 'Replace ToUpperInvariant() with this because these are equivalent on Telesto/Desktop
#Else
