    ...
    internal class ExpressionTreeRewriter : ExprVisitorBase
    {
        ...
        protected override EXPR VisitCALL(EXPRCALL expr)
        {
            ...
            else
            {
                ...
                //
                // CONSIDER: It might be better to rewrite the expression tree API so that it
        ...
        protected override EXPR VisitZEROINIT(EXPRZEROINIT expr)
        {
            ...
            if (expr.IsConstructor)
            {
                ...
                // CONSIDER: Move this realization out of the initial binding and into a later pass?
        ...
        protected virtual EXPR GenerateBuiltInBinaryOperator(EXPRBINOP expr)
        {
            ...
            switch (expr.kind)
            {
                ...
                // UNDONE: Tracked by DDBugs 109559
                // UNDONE: DivideChecked
        ...
        protected virtual EXPR GenerateUserDefinedUnaryOperator(EXPRUNARYOP expr)
        {
            ...
            switch (expr.kind)
            {
                ...
                // CONSIDER: Now that we have unary operators with methods in them, can we just make this an ExpressionKind.EK_NEG?
        ...
        protected virtual EXPR GenerateDelegateConstructor(EXPRCALL expr)
        {
            ...
            MethodSymbol createDelegateMethod = GetPreDefMethod(PREDEFMETH.PM_DELEGATE_CREATEDELEGATE_TYPE_OBJ_METHINFO);
            ...
            EXPR exprnull = GetExprFactory().CreateNull();
            ...
            EXPR param3 = GenerateConstant(GetExprFactory().CreateMethodInfo(funcptr.mwi));
            EXPR paramsList = GetExprFactory().CreateList(param1, param2, param3);
            EXPR Params = GenerateParamsArray(paramsList, PredefinedType.PT_EXPRESSION);
            EXPR call = GenerateCall(PREDEFMETH.PM_EXPRESSION_CALL, exprnull, methinfo, Params);
            EXPR pTypeOf = CreateTypeOf(expr.type);
            return GenerateCall(PREDEFMETH.PM_EXPRESSION_CONVERT, call, pTypeOf);
        }
