...
namespace Microsoft.CSharp.RuntimeBinder.Semantics
{
    ...
    /* ERROR */ new BinOpSig (PredefinedType.PT_ULONG,      PredefinedType.PT_LONG,     BinOpMask.Integer,  4, null,                    OpSigFlags.Value,       BinOpFuncKind.None          ),
    /* ERROR */ new BinOpSig (PredefinedType.PT_LONG,       PredefinedType.PT_ULONG,    BinOpMask.Integer,  3, null,                    OpSigFlags.Value,       BinOpFuncKind.None          ),
    ...
    /* ERROR */ new UnaOpSig( PredefinedType.PT_ULONG,      UnaOpMask.Minus,    3, null,            UnaOpFuncKind.None      ),
        ...
        // UNDONE: move this to the binding context
        ...
        ////////////////////////////////////////////////////////////////////////////////
        // REFACTOR: This code should be moved to !CSEE. We need to sort out binding of fixed var 
        // decls first however.
        ...
        protected EXPRQUESTIONMARK BindPtrToArray(EXPRLOCAL exprLoc, EXPR array)
        {
            ...
            // NOTE: The assignment needs to be inside the ExpressionKind.EK_QUESTIONMARK. See Whidbey bug #397859.
        ...
        protected EXPR bindIndexer(EXPR pObject, EXPR args, BindingFlag bindFlags)
        {
                            ...
                            (bindFlags & BindingFlag.BIND_BASECALL) != 0 ? (MemLookFlags.BaseCall | MemLookFlags.Indexer) : MemLookFlags.Indexer))
            {
                ...
                if (pSymbol != null)
                {
                    memgrp = GetExprFactory().CreateMemGroup((EXPRFLAG)mem.GetFlags(), // UNDONE: Gross cast here.
            ...
            EXPRMEMGRP grp = GetExprFactory().CreateMemGroup((EXPRFLAG)mem.GetFlags(), // UNDONE: Gross cast
        ...
        internal EXPR BindToProperty(EXPR pObject, PropWithType pwt, BindingFlag bindFlags, EXPR args, AggregateType pOtherType, EXPRMEMGRP pMemGroup)
        {
            ...
            // REVIEW  Shouldn't this check whether methSet is accessible before
            // setting EXPRFLAG.EXF_LVALUE?
        ...
        ////////////////////////////////////////////////////////////////////////////////
        // BADLY PLACED CODE: Move this method to operators.cpp
        ...
        EXPRCALL BindLiftedUDUnop(EXPR arg, CType typeArg, MethPropWithInst mpwi)
        {
        ...
        EXPRCALL BindUDUnopCall(EXPR arg, CType typeArg, MethPropWithInst mpwi)
        {
            CType typeRet = GetTypes().SubstType(mpwi.Meth().RetType, mpwi.GetType());
            ...
            switch (expr.kind)
            {
                case ExpressionKind.EK_PROP:
                    ...
                    if (!expr.asPROP().mwtSet)
                    {
                        ...
                        // See VSWhidbey bug 524600: If we have
                        ...
                        // REFACTOR: Ultimately it would be nice to change the binder so that
                        // REFACTOR: casts were not discarded so aggressively, exceptions to the
                        // REFACTOR: spec were more clearly represented in the code, and realization
                        // REFACTOR: of nullables was moved into a later pass.
        ...
        private EXPR AdjustMemberObject(SymWithType swt, EXPR pObject, out bool pfConstrained, out bool pIsMatchingStatic)
        {
            ...
            if (typeObj.IsTypeParameterType() || typeObj.IsAggregateType())
            {
                ...
                // avoid warning CS0649) - see Whidbey bug #434291.
        ...
        bool IsMatchingStatic(SymWithType swt, EXPR pObject)
        {
            Symbol pSym = swt.Sym;
        ...
        bool objectIsLvalue(EXPR pObject)
        {
            return (
        ...
        public static void RemapToOverride(SymbolLoader symbolLoader, SymWithType pswt, CType typeObj)
        {
            ...
            // REVIEW  : this isn't going to work for calling arrays or Enum methods
        ...
        private static readonly PredefinedType[] rgptIntOp = 
        { 
        ...
        internal CType chooseArrayIndexType(EXPR args)
        {
            ...
            {
                CType type = GetReqPDT(rgptIntOp[ipt]);
        ...
        readonly static private PredefinedName[] EK2NAME =
        {
        ...
        {
            Debug.Assert(ek >= ExpressionKind.EK_FIRSTOP && (ek - ExpressionKind.EK_FIRSTOP) < (int)EK2NAME.Length);
            return GetSymbolLoader().GetNameManager().GetPredefName(EK2NAME[ek - ExpressionKind.EK_FIRSTOP]);
        }
