...
namespace Microsoft.CSharp.RuntimeBinder.Semantics
{
    ...
    internal partial class ExpressionBinder
    {
        ...
        static private readonly byte[,] simpleTypeConversions =
        {
...
/*   U8 */ {  EXP , EXP , EXP , EXP , IMP , IMP , IUD, EXP , NO , EXP , EXP , EXP , ID  },
        ...
        private static ConvKind GetConvKind(PredefinedType ptSrc, PredefinedType ptDst)
        {
            ...
            {
                return (ConvKind)(simpleTypeConversions[(int)ptSrc, (int)ptDst] & CONV_KIND_MASK);
            }
        ...
        private static bool isUserDefinedConversion(PredefinedType ptSrc, PredefinedType ptDst)
        {
            if ((int)ptSrc < NUM_SIMPLE_TYPES && (int)ptDst < NUM_SIMPLE_TYPES)
            {
                return 0 != (simpleTypeConversions[(int)ptSrc, (int)ptDst] & UDC);
            }
            return false;
        }
        ...
        static private readonly byte[,] simpleTypeBetter =
        {
...
/* OBJECT */{right  ,right  ,right  ,right  ,right  ,right  ,right  ,right  ,right  ,right  ,right  ,right  ,right  ,right  ,right  ,same   }
...
#if DEBUG
        static private volatile bool fCheckedBetter = false;
        private void CheckBetterTable()
        {
            ...
            {
                Debug.Assert(simpleTypeBetter[i, i] == same);
                for (int j = 0; j < i; j++)
                {
                    Debug.Assert(simpleTypeBetter[i, j] != same && simpleTypeBetter[j, i] != same);
                    Debug.Assert(
                        (simpleTypeBetter[i, j] == left && simpleTypeBetter[j, i] == right) ||
                        (simpleTypeBetter[i, j] == right && simpleTypeBetter[j, i] == left) ||
                        (simpleTypeBetter[i, j] == neither && simpleTypeBetter[j, i] == neither));
                    Debug.Assert(
                        GetOptPDT((PredefinedType)i) == null ||
                        GetOptPDT((PredefinedType)j) == null ||
                        (!canConvert(GetOptPDT((PredefinedType)i), GetOptPDT((PredefinedType)j), CONVERTTYPE.NOUDC) || simpleTypeBetter[i, j] == left) &&
                        (!canConvert(GetOptPDT((PredefinedType)j), GetOptPDT((PredefinedType)i), CONVERTTYPE.NOUDC) || simpleTypeBetter[j, i] == left));
                }
            }
            fCheckedBetter = true;
        }
        ...
        private BetterType WhichSimpleConversionIsBetter(PredefinedType pt1, PredefinedType pt2)
        {
            ...
            return (BetterType)simpleTypeBetter[(int)pt1, (int)pt2];
        }
        ...
        ***************************************************************************************************/
        ...
        public bool BindGrpConversion(EXPRMEMGRP grp, CType typeDst, bool needDest, out EXPRCALL pexprDst, bool fReportErrors)
        {
            ...
            if (methInvoke.Params.Size < @params.Size && mwiWrap.Meth().IsExtension())
            {
                ...
                {
                    // We should issue a better message here. This is tracked by DevDiv Bugs 142866
        ...
        /***************************************************************************************************
            ...
            REVIEW : Consider caching the results of this. Performing this over and over again
            ...
            for (int iats = 0; iats < cats; iats++)
            {
                for (AggregateType atsCur = rgats[iats]; atsCur != null && atsCur.getAggregate().HasConversion(this.GetSymbolLoader()); atsCur = atsCur.GetBaseClass())
                {
                    ...
                    for (MethodSymbol convCur = aggCur.GetFirstUDConversion(); convCur != null; convCur = convCur.ConvNext())
                    {
                        ...
                        {
                            // DDBugs 123512 - If we have a user-defined conversion that 
                        ...
                        { // REVIEW : Can this check be removed?
                            FUNDTYPE ftFrom;
                            FUNDTYPE ftTo;
