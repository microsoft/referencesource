    ...
    internal class MethodTypeInferrer
    {
        ...
        */
            ...
            out TypeArray ppInferredTypeArguments)
        {
            ...
            {
                // CONSIDER: fill in with error symbols as when inference fails?
                return false;
            }
        ...
        {
            this.binder = exprBinder;
            this.symbolLoader = symLoader;
            ...
            this.pFixedResults = new CType[pMethodTypeParameters.size];
            this.pLowerBounds = new List<CType>[pMethodTypeParameters.size];
            this.pUpperBounds = new List<CType>[pMethodTypeParameters.size];
            this.pExactBounds = new List<CType>[pMethodTypeParameters.size];
            for (int iBound = 0; iBound < pMethodTypeParameters.size; ++iBound)
            {
                pLowerBounds[iBound] = new List<CType>();
                pUpperBounds[iBound] = new List<CType>();
                pExactBounds[iBound] = new List<CType>();
            }
            this.ppDependencies = null;
        }

        ////////////////////////////////////////////////////////////////////////////////

        TypeArray GetResults() 
        {
            ...
            for (int iParam = 0; iParam < pMethodTypeParameters.size; iParam++)
            {
                ...
                // (DevDiv Bugs #74826) We get an ErrorType with a null nameText
                // for a CType variable that we couldn't infer.
                if (pFixedResults[iParam] != null)
                {
                    if (!pFixedResults[iParam].IsErrorType())
                    {
                        continue;
                    }

                    Name pErrorTypeName = pFixedResults[iParam].AsErrorType().nameText;
                ...
                pFixedResults[iParam] = GetTypeManager().GetErrorType(
                                        null/*pParentType*/,
                                        null,
                                        pMethodTypeParameters.ItemAsTypeParameterType(iParam).GetName(),
        ...
        bool IsUnfixed(int iParam) 
        {
            Debug.Assert(0 <= iParam);
            Debug.Assert(iParam < pMethodTypeParameters.size);
            return pFixedResults[iParam] == null;
        }

        ////////////////////////////////////////////////////////////////////////////////

        bool IsUnfixed(TypeParameterType pParam) 
        {
            Debug.Assert(pParam != null);
            Debug.Assert(pParam.IsMethodTypeParameter());
            int iParam = pParam.GetIndexInTotalParameters();
            Debug.Assert(pMethodTypeParameters.ItemAsTypeParameterType(iParam) == pParam);
        ...
        void AddLowerBound(TypeParameterType pParam, CType pBound)
        {
            Debug.Assert(IsUnfixed(pParam));
            int iParam = pParam.GetIndexInTotalParameters();
            if (!pLowerBounds[iParam].Contains(pBound))
            {
                pLowerBounds[iParam].Add(pBound);
            }
        }

        ////////////////////////////////////////////////////////////////////////////////

        void AddUpperBound(TypeParameterType pParam, CType pBound)
        {
            Debug.Assert(IsUnfixed(pParam));
            int iParam = pParam.GetIndexInTotalParameters();
            if (!pUpperBounds[iParam].Contains(pBound))
            {
                pUpperBounds[iParam].Add(pBound);
            }
        }

        ////////////////////////////////////////////////////////////////////////////////

        void AddExactBound(TypeParameterType pParam, CType pBound)
        {
            Debug.Assert(IsUnfixed(pParam));
            int iParam = pParam.GetIndexInTotalParameters();
            if (!pExactBounds[iParam].Contains(pBound))
            {
                pExactBounds[iParam].Add(pBound);
            }
        }

        ////////////////////////////////////////////////////////////////////////////////

        bool HasBound(int iParam) 
        {
            Debug.Assert(0 <= iParam);
            Debug.Assert(iParam < pMethodTypeParameters.size);
            return !pLowerBounds[iParam].IsEmpty() ||
                !pExactBounds[iParam].IsEmpty() || 
                !pUpperBounds[iParam].IsEmpty();
        }

        ////////////////////////////////////////////////////////////////////////////////

        TypeArray GetFixedDelegateParameters(AggregateType pDelegateType)
        {
            ...
            CType[] ppMethodParameters = new CType[pMethodTypeParameters.size];
            for (int iParam = 0 ; iParam < pMethodTypeParameters.size; iParam++)
            {
                TypeParameterType pParam = pMethodTypeParameters.ItemAsTypeParameterType(iParam);
                ppMethodParameters[iParam] = IsUnfixed(iParam) ? pParam : pFixedResults[iParam];
            }
            SubstContext subsctx = new SubstContext(pClassTypeArguments.ToArray(), pClassTypeArguments.size,
                ppMethodParameters, pMethodTypeParameters.size);
        ...
        static bool IsReallyAType(CType pType)
        {
        ...
        void InferTypeArgsFirstPhase()
        {
            ...
            for (int iArg = 0; iArg < pMethodArguments.carg; iArg++)
            {
                ...
                EXPR pExpr = pMethodArguments.prgexpr[iArg];
                ...
                CType pDest = pMethodFormalParameterTypes.Item(iArg);
                ...
                CType pSource = pExpr.RuntimeObjectActualType != null
                    ...
                    : pMethodArguments.types.Item(iArg);
        ...
        void MakeOutputTypeInferences()
        {
            ...
            for (int iArg = 0; iArg < pMethodArguments.carg; iArg++)
            {
                ...
                {
                    CType pSource = pMethodArguments.types.Item(iArg);
                    if (pSource.IsParameterModifierType())
                    {
                        pSource = pSource.AsParameterModifierType().GetParameterType();
                    }
        ...
        NewInferenceResult FixNondependentParameters()
        {
            ...
            bool[] pNeedsFixing = new bool[pMethodTypeParameters.size];
            int iParam;
            NewInferenceResult res = NewInferenceResult.NoProgress;
            for (iParam = 0; iParam < pMethodTypeParameters.size; iParam++)
            {
        ...
        // Input types
        //
        bool DoesInputTypeContain(EXPR pSource, CType pDest,
        ...
        bool HasUnfixedParamInInputType(EXPR pSource, CType pDest)
        {
            for (int iParam = 0; iParam < pMethodTypeParameters.size; iParam++)
            {
                if (IsUnfixed(iParam))
                {
                    if (DoesInputTypeContain(pSource, pDest,
                        pMethodTypeParameters.ItemAsTypeParameterType(iParam)))
                    {
                        return true;
                    }
                }
        ...
        // Output types
        //
        bool DoesOutputTypeContain(EXPR pSource, CType pDest,
            TypeParameterType pParam)
        {
            // SPEC: If E is a method group or an anonymous function and T is a delegate
        ...
        bool HasUnfixedParamInOutputType(EXPR pSource, CType pDest)
        {
            for (int iParam = 0; iParam < pMethodTypeParameters.size; iParam++)
            {
                if (IsUnfixed(iParam))
        ...
        bool DependsDirectlyOn(int iParam, int jParam)
        {
            Debug.Assert(0 <= iParam && iParam < pMethodTypeParameters.size);
            Debug.Assert(0 <= jParam && jParam < pMethodTypeParameters.size);
            ...
            for (int iArg = 0; iArg < pMethodArguments.carg; iArg++)
            {
                ...
                if (DoesInputTypeContain(pExpr, pDest,
                        pMethodTypeParameters.ItemAsTypeParameterType(jParam)) &&
        ...
        void InitializeDependencies()
        {
            ...
            Debug.Assert(ppDependencies == null);
            ppDependencies = new Dependency[pMethodTypeParameters.size, pMethodTypeParameters.size];
            for (int iParam = 0; iParam < pMethodTypeParameters.size; ++iParam)
            {
                for (int jParam = 0; jParam < pMethodTypeParameters.size; ++jParam)
                {
                    if (DependsDirectlyOn(iParam, jParam))
                    {
                        ppDependencies[iParam, jParam] = Dependency.Direct;
                    }
                }
            }
        ...
        bool DependsOn(int iParam, int jParam)
        {
            Debug.Assert(ppDependencies != null);
            ...
            return 0 != ((ppDependencies[iParam, jParam]) & Dependency.DependsMask);
        }

        ////////////////////////////////////////////////////////////////////////////////

        bool DependsTransitivelyOn(int iParam, int jParam)
        {
            Debug.Assert(ppDependencies != null);
            Debug.Assert(0 <= iParam && iParam < pMethodTypeParameters.size);
            Debug.Assert(0 <= jParam && jParam < pMethodTypeParameters.size);
            ...
            for(int kParam = 0 ; kParam < pMethodTypeParameters.size; ++kParam)
            {
                if (0 != ((ppDependencies[iParam, kParam]) & Dependency.DependsMask) &&
                    0 != ((ppDependencies[kParam, jParam]) & Dependency.DependsMask))
                {
                    return true;
                }            
            }
        ...
        bool DeduceDependencies()
        {
            ...
            for (int iParam = 0; iParam < pMethodTypeParameters.size; ++iParam)
            {
                for (int jParam = 0; jParam < pMethodTypeParameters.size; ++jParam)
                {
                    if (ppDependencies[iParam, jParam] == Dependency.Unknown)
                    {
                        if (DependsTransitivelyOn(iParam, jParam))
                        {
                            ppDependencies[iParam, jParam] = Dependency.Indirect;
                            madeProgress = true;
                        }
                    }
                }
        ...
        void SetUnknownsToNotDependent()
        {
            Debug.Assert(ppDependencies != null);
            for (int iParam = 0; iParam < pMethodTypeParameters.size; ++iParam)
            {
                for (int jParam = 0; jParam < pMethodTypeParameters.size; ++jParam)
                {
                    if (ppDependencies[iParam, jParam] == Dependency.Unknown)
                    {
                        ppDependencies[iParam, jParam] = Dependency.NotDependent;
                    }
                }
            }
        }
        ...
        void SetIndirectsToUnknown()
        {
            ...
            for (int iParam = 0; iParam < pMethodTypeParameters.size; ++iParam)
            {
                for (int jParam = 0; jParam < pMethodTypeParameters.size; ++jParam)
                {
                    if (ppDependencies[iParam, jParam] == Dependency.Indirect)
                    {
                        ppDependencies[iParam, jParam] = Dependency.Unknown;
                    }
                }
            }
        }
        ...
        void UpdateDependenciesAfterFix(int iParam)
        {
            ...
            {
                ppDependencies[iParam, jParam] = Dependency.NotDependent;
                ppDependencies[jParam, iParam] = Dependency.NotDependent;
            }
        ...
        void OutputTypeInference(EXPR pExpr, CType pSource, CType pDest)
        {
        ...
        bool MethodGroupReturnTypeInference(EXPR pSource, CType pType)
        {
            // SPEC:  Otherwise, if E is a method group and T is a delegate CType or
            // SPEC:   expression tree CType with parameter types T1...Tk and return
            // SPEC:   CType Tb and overload resolution of E with the types T1...Tk
            ...
            var argsBinder = new ExpressionBinder.GroupToArgsBinder(binder, 0/* flags */, pSource.asMEMGRP(), argInfo, null, false, pDelegateType);
        ...
        // Exact inferences
        //
        void ExactInference(CType pSource, CType pDest)
        {
        ...
        bool ExactTypeParameterInference(CType pSource, CType pDest)
        {
        ...
        bool ExactArrayInference(CType pSource, CType pDest)
        {
            // SPEC:  Otherwise, if U is an array CType UE[...] and V is an array CType VE[...]
            // SPEC:   of the same rank then an exact inference from UE to VE is made.
        ...
        bool ExactNullableInference(CType pSource, CType pDest)
        {
        ...
        bool ExactConstructedInference(CType pSource, CType pDest)
        {
            // SPEC:  Otherwise, if V is a constructed CType C<V1...Vk> and U is a constructed
        ...
        //
        void LowerBoundInference(CType pSource, CType pDest)
        {
            ...
            // UNDONE:
            // UNDONE:  Otherwise, if V is nullable CType V1? and U is a non-nullable 
            // UNDONE:   struct CType then an exact inference is made from U to V1.
            // UNDONE:
            ...
            // UNDONE: a delegate CType assumes that if method CType inference succeeds,
        ...
        bool LowerBoundTypeParameterInference(CType pSource, CType pDest)
        {
            // SPEC:  If V is one of the unfixed Xi then U is added to the set of bounds
            // SPEC:   for Xi.
            if (pDest.IsTypeParameterType())
        ...
        bool LowerBoundArrayInference(CType pSource, CType pDest)
        {
            // SPEC:  Otherwise, if U is an array CType Ue[...] and V is either an array
            // SPEC:   CType Ve[...] of the same rank, or if U is a one-dimensional array
        ...
        bool LowerBoundConstructedInference(CType pSource, CType pDest)
        {
        ...
        bool LowerBoundClassInference(CType pSource, AggregateType pDest)
        {
        ...
        bool LowerBoundInterfaceInference(CType pSource, AggregateType pDest)
        {
        ...
        void LowerBoundTypeArgumentInference(
            AggregateType pSource, AggregateType pDest)
        {
        ...
        //
        void UpperBoundInference(CType pSource, CType pDest)
        {
        ...
        bool UpperBoundTypeParameterInference(CType pSource, CType pDest)
        {
        ...
        bool UpperBoundArrayInference(CType pSource, CType pDest)
        {
            // SPEC:  Otherwise, if V is an array CType Ve[...] and U is an array
            // SPEC:   CType Ue[...] of the same rank, or if V is a one-dimensional array
        ...
        bool UpperBoundConstructedInference(CType pSource, CType pDest)
        {
            if (!pSource.IsAggregateType())
            {
                return false;
        ...
        bool UpperBoundClassInference(AggregateType pSource, CType pDest)
        {
        ...
        bool UpperBoundInterfaceInference(AggregateType pSource, CType pDest)
        {
        ...
        void UpperBoundTypeArgumentInference(
            AggregateType pSource, AggregateType pDest)
        {
            // SPEC: The choice of inference for the i-th CType argument is made
            // SPEC: based on the declaration of the i-th CType parameter of C, as
        ...
        bool Fix(int iParam)
        {
            ...
            if (pExactBounds[iParam].IsEmpty())
            {
                ...
                foreach (CType pCurrent in pLowerBounds[iParam])
                {
                ...
                foreach (CType pCurrent in pUpperBounds[iParam])
                {
                    if (!typeSet.Contains(pCurrent))
                    {
                        typeSet.Add(pCurrent);
            ...
            {
                initialCandidates.Add(pExactBounds[iParam].Head());
            }
            ...
            foreach (CType pBound in pLowerBounds[iParam])
            {
                List<CType> removeList = new List<CType>();
                foreach (CType pCandidate in initialCandidates)
                {
                    if (pBound != pCandidate && !binder.canConvert(pBound, pCandidate))
                    {
            ...
            foreach (CType pBound in pUpperBounds[iParam])
            {
                List<CType> removeList = new List<CType>();
                foreach (CType pCandidate in initialCandidates)
                {
                    if (pBound != pCandidate && !binder.canConvert(pCandidate, pBound))
                    {
                        removeList.Add(pCandidate);
                    }
                }
            ...
            foreach (CType pCandidate in initialCandidates)
            {
                ...
                {
                    if (pCandidate != pCandidate2 && !binder.canConvert(pCandidate2, pCandidate))
                    {
                        goto OuterBreak;
                    }
                }
        ...
        OuterBreak:
            ...
            if (GetTypeManager().GetBestAccessibleType(binder.GetSemanticChecker(), binder.GetContext(), pBest, out pBestAccessible))
            {
        ...
        //
        bool InferForMethodGroupConversion()
        {
        ...
        bool CanInferExtensionObject()
        {
            Debug.Assert(pMethodFormalParameterTypes != null);
            Debug.Assert(pMethodFormalParameterTypes.size >= 1);
            Debug.Assert(pMethodArguments != null);
            Debug.Assert(pMethodArguments.carg >= 1);
            CType pDest = pMethodFormalParameterTypes.Item(0);
            CType pSource = pMethodArguments.types.Item(0);
            if (pDest.IsParameterModifierType())
            {
                pDest = pDest.AsParameterModifierType().GetParameterType();
            }
