...
namespace Microsoft.CSharp.RuntimeBinder.Syntax
{
    ...
    {
        class KnownName : Name
        {
            PredefinedName id = PredefinedName.PN_COUNT;
        ...
        private static NameTable _knownNames;
        ...
        private void InitKnownNames()
        {
            if (_knownNames == null)
            {
                ...
                Debug.Assert(_predefinedNames.Length == (int)PredefinedName.PN_COUNT);
                for (int i = 0, n = _predefinedNames.Length; i < n; i++)
                {
                    Debug.Assert((int)_predefinedNames[i].PredefinedName == i);
                    Name name = _predefinedNames[i];
                    tmp.Add(name);
                }

                // add all other names
                foreach (KnownName name in _otherNames)
                {
                    tmp.Add(name);
                }

                Interlocked.CompareExchange<NameTable>(ref _knownNames, tmp, null);
            }
        }

        private static KnownName[] _predefinedNames = new KnownName[(int)PredefinedName.PN_COUNT] {
        ...
        private static KnownName[] _otherNames = new KnownName[] {
            new KnownName("true"),
            new KnownName("false"),
            new KnownName("null"),
            new KnownName("base"),
