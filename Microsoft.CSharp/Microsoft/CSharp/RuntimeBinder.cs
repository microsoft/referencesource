...
namespace Microsoft.CSharp.RuntimeBinder
{
    internal class RuntimeBinder
    {
        ...
        private SymbolLoader SymbolLoader { get { return m_semanticChecker.GetSymbolLoader(); } }
        ...
        private void Reset()
        {
            ...
            m_symbolTable = new SymbolTable(
                ...
                nameManager,
                m_semanticChecker.GetTypeManager(),
                bsymmgr,
                m_semanticChecker,
                infile);
            ...
            m_bindingContext = BindingContext.CreateInstance(
                m_semanticChecker,
                m_exprFactory,
                m_outputContext,
                m_nameGenerator,
            ...
            out DynamicMetaObject deferredBinding)
        {
            ...
            lock (m_bindLock)
            {
                ...
                catch (ResetBindException)
                {
                    ...
                    {
                        Reset();
                        Debug.Assert(false, "RuntimeBinderInternalCompilerException", "More than one symbol table name collision in a single binding");
            ...
            out DynamicMetaObject deferredBinding)
        {
            ...
            if (payload is CSharpInvokeMemberBinder)
            {
                ...
                Debug.Assert(m_bindingContext.ContextForMemberLookup() != null);
                SymWithType swt = m_symbolTable.LookupMember(
                        callPayload.Name,
                        callingObject,
                        m_bindingContext.ContextForMemberLookup(),
                        arity,
                        mem,
        ...
        private void InitializeCallingContext(DynamicMetaObjectBinder payload)
        {
            ...
            if (t != null)
            {
                AggregateSymbol agg = m_symbolTable.GetCTypeFromType(t).AsAggregateType().GetOwningAggregate();
                m_bindingContext.m_pParentDecl = m_semanticChecker.GetGlobalSymbolFactory().CreateAggregateDecl(agg, null);
            }
            else
            {
            ...
            m_bindingContext.CheckedConstant = bChecked;
            m_bindingContext.CheckedNormal = bChecked;
        }
            ...
            EXPR pResult)
        {
            ...
            EXPR exprTree = ExpressionTreeRewriter.Rewrite(boundLambda, m_exprFactory, SymbolLoader);
                ...
                DynamicMetaObject[] args)
        {
            ...
            {
                Debug.Assert(false, "RuntimeBinderInternalCompilerException", "Unknown payload kind");
                throw Error.InternalCompilerError();
            }
            getArgumentType = (p, argInfo, param, arg, index) =>
                {
                    ...
                    else if (!argInfo.UseCompileTimeType)
                    {
                        ...
                        CType actualType = m_symbolTable.GetCTypeFromType(t);
                        CType bestType;

                        bool res = m_semanticChecker.GetTypeManager().GetBestAccessibleType(m_semanticChecker, m_bindingContext, actualType, out bestType);
                        if (!res)
                        {
            ...
            ArgumentObject[] arguments)
        {
            ...
            if ((callOrInvoke = payload as ICSharpInvokeOrInvokeMemberBinder) != null)
            {
                ...
                if (callOrInvoke.StaticCall)
                {
                    ...
                    {
                        Debug.Assert(false, "RuntimeBinderInternalCompilerException", "Cannot make static call without specifying a type");
                        throw Error.InternalCompilerError();
                    }
                ...
                m_symbolTable.PopulateSymbolTableWithName(
                    callOrInvoke.Name,
            ...
            else if (payload is CSharpBinaryOperationBinder)
            {
                ...
                {
                    Debug.Assert(false, "RuntimeBinderInternalCompilerException", "Unknown operator: " + op.Operation);
                    throw Error.InternalCompilerError();
                }
                m_symbolTable.PopulateSymbolTableWithName(
        ...
        private void AddConversionsForArguments(ArgumentObject[] arguments)
        {
            ...
            {
                m_symbolTable.AddConversionsForType(arg.Type);
            }
        }
                ...
                Debug.Assert(arguments.Length == 1);
                {
                    ...
                    switch (conversion.ConversionKind)
                    {
                        ...
                        case CSharpConversionKind.ArrayCreationConversion:
                            ...
                            break;
                        default:
                            Debug.Assert(false, "RuntimeBinderInternalCompilerException", "Unknown conversion kind");
                            throw Error.InternalCompilerError();
                    }
                }
            }
            ...
            Dictionary<int, LocalVariableSymbol> dictionary)
        {
            ...
            {
                CType type = m_symbolTable.GetCTypeFromType(parameter.Type);
                    ...
                    (arguments[i].Info.IsByRef || arguments[i].Info.IsOut))
                {
                    ...
                    {
                        type = m_semanticChecker.GetTypeManager().GetParameterModifier(type, arguments[i].Info.IsOut);
                    }
                }
                LocalVariableSymbol local = m_semanticChecker.GetGlobalSymbolFactory().CreateLocalVar(m_semanticChecker.GetNameManager().Add("p" + i), pScope, type);
            ...
            EXPR call)
        {
            ...
            LocalVariableSymbol thisLocal = m_semanticChecker.GetGlobalSymbolFactory().CreateLocalVar(m_semanticChecker.GetNameManager().Add("this"), pScope, m_symbolTable.GetCTypeFromType(typeof(object)));
            thisLocal.isThis = true;
            EXPRBOUNDLAMBDA boundLambda = m_exprFactory.CreateAnonymousMethod(delegateType);
            EXPRUNBOUNDLAMBDA unboundLambda = m_exprFactory.CreateLambda();
            ...
            EXPRRETURN returnStatement = m_exprFactory.CreateReturn(0, pScope, call);
            EXPRBLOCK block = m_exprFactory.CreateBlock(null, returnStatement, pScope);
        ...
        private EXPR CreateLocal(Type type, bool bIsOut, LocalVariableSymbol local)
        {
            ...
            if (bIsOut)
            {
                ...
                ctype = m_semanticChecker.GetTypeManager().GetParameterModifier(
                    ctype.AsParameterModifierType().GetParameterType(),
                    true);
            }

            // If we can convert, do that. If not, cast it.
            EXPRLOCAL exprLocal = m_exprFactory.CreateLocal(EXPRFLAG.EXF_LVALUE, local);
            EXPR result = m_binder.tryConvert(exprLocal, ctype);
            if (result == null)
            {
                result = m_binder.mustCast(exprLocal, ctype);
            }
            ...
            if (arguments != null)
            {
                for (int i = startIndex; i < endIndex; i++)
                {
                    ...
                    else
                    {
                        ...
                        m_exprFactory.AppendItemToList(arg, ref args, ref last);
                    }
                }
            }
            return args;
        ...
        private EXPR CreateArgumentEXPR(ArgumentObject argument, LocalVariableSymbol local)
        {
            ...
            if (argument.Info.LiteralConstant)
            {
                if (argument.Value == null)
                {
                    ...
                    {
                        arg = m_exprFactory.CreateConstant(m_symbolTable.GetCTypeFromType(argument.Type), new CONSTVAL());
                    }
                    else
                    {
                        arg = m_exprFactory.CreateNull();
                    }
                }
                else
                {
                    arg = m_exprFactory.CreateConstant(m_symbolTable.GetCTypeFromType(argument.Type), new CONSTVAL(argument.Value));
                }
            }
            else
            {
            ...
            if (argument.Info.NamedArgument)
            {
                ...
                arg = m_exprFactory.CreateNamedArgumentSpecification(SymbolTable.GetName(argument.Info.Name, m_semanticChecker.GetNameManager()), arg);
            }
            ...
            if (!argument.Info.UseCompileTimeType && argument.Value != null)
            {
                ...
                arg.RuntimeObjectActualType = m_symbolTable.GetCTypeFromType(argument.Value.GetType());
            }
            ...
            SYMKIND kind)
        {
            ...
            {
                callingType = m_semanticChecker.GetSymbolLoader().GetReqPredefType(PredefinedType.PT_ARRAY);
            }
            else if (callingObject.type.IsNullableType())
            {
                callingType = callingObject.type.AsNullableType().GetAts(m_semanticChecker.GetSymbolLoader().GetErrorContext());
            }
            ...
            {
                if (m_symbolTable.AggregateContainsMethod(t.GetOwningAggregate(), Name, mask))
                {
                    callingTypes.Add(t);
                }
            ...
            if (callingType.IsWindowsRuntimeType())
            {
                ...
                for (int i = 0; i < collectioniFaces.size; i++)
                {
                    ...
                    if (m_symbolTable.AggregateContainsMethod(t.AsAggregateType().GetOwningAggregate(), Name, mask))
                    {
                        callingTypes.Add(t);
                    }
                }
            ...
            {
                typeArgumentsAsTypeArray = m_semanticChecker.getBSymmgr().AllocParams(
                    m_symbolTable.GetCTypeArrayFromTypes(typeArguments));
            }
            EXPRMEMGRP memgroup = m_exprFactory.CreateMemGroup(// Tree
                flags, name, typeArgumentsAsTypeArray, kind, callingType, null, null, new CMemberLookupResults(
                    m_semanticChecker.getBSymmgr().AllocParams(callingTypes.Count, callingTypes.ToArray()),
                    name));
            ...
            BindingFlag flags)
        {
            ...
            return m_binder.BindToProperty(// For a static property instance, dont set the object.
        ...
        private EXPR CreateIndexer(SymWithType swt, EXPR callingObject, EXPR arguments, BindingFlag bindFlags)
        {
            ...
            EXPR result = m_binder.BindMethodGroupToArguments(bindFlags, memgroup, arguments);
        ...
        {
            return m_binder.BindArrayIndexCore(0, callingObject, optionalIndexerArguments);
        }
            ...
            EXPR callingObject)
        {
            ...
            EXPR field = m_binder.BindToField(callingObject.isCLASS() ? null : callingObject, fwt, 0);
            return field;
        }
            ...
            EXPR callingObject)
        {
            ...
            EXPREVENT e = m_exprFactory.CreateEvent(eventSymbol.type, callingObject, new EventWithType(eventSymbol, swt.GetType()));
            return e;
        }
        #endregion
            ...
            Dictionary<int, LocalVariableSymbol> dictionary)
        {
            ...
            if (payload.StaticCall)
            {
                ...
                callingObject = m_exprFactory.CreateClass(m_symbolTable.GetCTypeFromType(t), null, t.ContainsGenericParameters ?
                        m_exprFactory.CreateTypeArguments(SymbolLoader.getBSymmgr().AllocParams(m_symbolTable.GetCTypeArrayFromTypes(t.GetGenericArguments())), null) : null);
            }
            else
            {
                ...
                callingObject = m_binder.mustConvert(
                    CreateArgumentEXPR(arguments[0], dictionary[0]),
                    m_symbolTable.GetCTypeFromType(arguments[0].Type));
            ...
            Dictionary<int, LocalVariableSymbol> dictionary)
        {
            ...
            {
                Debug.Assert(false, "RuntimeBinderInternalCompilerException", "Unexpected type returned from lookup");
                throw Error.InternalCompilerError();
            }
            ...
            if ((payload.Flags & CSharpCallFlags.EventHookup) != 0)
            {
                ...
                SymWithType swtEvent = m_symbolTable.LookupMember(
            ...
            pResult = m_binder.BindMethodGroupToArguments(// Tree
        ...
        private EXPR BindWinRTEventAccessor(EventWithType ewt, EXPR callingObject, ArgumentObject[] arguments, Dictionary<int, LocalVariableSymbol> dictionary, bool isAddAccessor)
        {
            ...
            EXPRMEMGRP removeMethGrp = m_exprFactory.CreateMemGroup(callingObject, removemwi); 
            removeMethGrp.flags &= ~EXPRFLAG.EXF_USERCALLABLE;
            Type actionType = Expression.GetActionType(typeof(EventRegistrationToken));
            EXPR removeMethArg = m_binder.mustConvert(removeMethGrp, m_symbolTable.GetCTypeFromType(actionType));
            ...
            if (isAddAccessor)
            {
                ...
                EXPRMEMGRP addMethGrp = m_exprFactory.CreateMemGroup(callingObject, addmwi); 
                addMethGrp.flags &= ~EXPRFLAG.EXF_USERCALLABLE;
                Type funcType = Expression.GetFuncType(evtType, typeof(EventRegistrationToken));
                EXPR addMethArg = m_binder.mustConvert(addMethGrp, m_symbolTable.GetCTypeFromType(funcType));

                args = m_exprFactory.CreateList(addMethArg, removeMethArg, delegateVal);
                methodName = SymbolLoader.GetNameManager().GetPredefName(PredefinedName.PN_ADDEVENTHANDLER).Text;
            }
            else
            {
                args = m_exprFactory.CreateList(removeMethArg, delegateVal);
            ...
            m_symbolTable.PopulateSymbolTableWithName( methodName, new List<Type> { evtType }, typeof(WindowsRuntimeMarshal));
            EXPRCLASS marshalClass = m_exprFactory.CreateClass(m_symbolTable.GetCTypeFromType(typeof(WindowsRuntimeMarshal)), null, null);
            EXPRMEMGRP addEventGrp = CreateMemberGroupEXPR(methodName, new List<Type> { evtType }, marshalClass, SYMKIND.SK_MethodSymbol);
            EXPR expr = m_binder.BindMethodGroupToArguments(
        ...
        private void CheckForConditionalMethodError(EXPR pExpr)
        {
            ...
            if (pExpr.isCALL())
            {
                ...
                object[] conditions = method.AssociatedMemberInfo.GetCustomAttributes(typeof(ConditionalAttribute), false);
        ...
        private EXPR ReorderArgumentsForNamedAndOptional(EXPR callingObject, EXPR pResult)
        {
            ...
            m_binder.FillInArgInfoFromArgList(argInfo, arguments);
                ...
                SymbolLoader);
            {
                ...
                for (int i = argInfo.carg - 1; i >= 0; i--)
                {
                    ...
                    pArg = m_binder.tryConvert(pArg, parameters[i]);
                    if (pList == null)
                    {
                        pList = pArg;
                    }
                    ...
                    {
                        pList = m_exprFactory.CreateList(pArg, pList);
                    }
                }
                if (pResult.isCALL())
                {
            ...
            Dictionary<int, LocalVariableSymbol> dictionary)
        {
            ...
            if (op == OperatorKind.OP_TRUE || op == OperatorKind.OP_FALSE)
            {
                ...
                EXPR result = m_binder.tryConvert(arg1, SymbolLoader.GetReqPredefType(PredefinedType.PT_BOOL));
                if (result != null && op == OperatorKind.OP_FALSE)
                {
                    // If we can convert to bool, we need to negate the thing if we're looking for false.
                    result = m_binder.BindStandardUnaryOperator(OperatorKind.OP_LOGNOT, result);
                }

                if (result == null)
                {
                    result = m_binder.bindUDUnop(op == OperatorKind.OP_TRUE ? ExpressionKind.EK_TRUE : ExpressionKind.EK_FALSE, arg1);
                }
                ...
                if (result == null)
                {
                    result = m_binder.mustConvert(arg1, SymbolLoader.GetReqPredefType(PredefinedType.PT_BOOL));
                }
                return result;
            }
            return m_binder.BindStandardUnaryOperator(op, arg1);
        }
        #endregion
                ...
                Dictionary<int, LocalVariableSymbol> dictionary)
        {
            ...
            return m_binder.BindStandardBinop(ek, arg1, arg2);
        }
        #endregion
        ...
        private static OperatorKind GetOperatorKind(ExpressionType p, bool bIsLogical)
        {
            ...
            {
                default:
                    Debug.Assert(false, "RuntimeBinderInternalCompilerException", "Unknown operator: " + p);
                    throw Error.InternalCompilerError();
            ...
            bool fEventsPermitted)
        {
            ...
            EXPR callingObject = argument.Info.IsStaticType ?
                m_exprFactory.CreateClass(m_symbolTable.GetCTypeFromType(argument.Value as Type), null, null) :
            ...
            MemberLookup mem = new MemberLookup();
            SymWithType swt = m_symbolTable.LookupMember(name, callingObject, m_bindingContext.ContextForMemberLookup(), 0, mem, false, false);
            if (swt == null)
            {
            ...
            bool bIsArrayCreationConversion)
        {
            ...
            CType destinationType = m_symbolTable.GetCTypeFromType(returnType);
            ...
            if (bIsArrayCreationConversion)
            {
                ...
                CType pDestType = m_binder.chooseArrayIndexType(argument);
                if (null == pDestType)
                {
                    pDestType = SymbolLoader.GetReqPredefType(PredefinedType.PT_INT, true);
                }

                return m_binder.mustCast(
                    m_binder.mustConvert(argument, pDestType),
                    destinationType, 
                    CONVERTTYPE.CHECKOVERFLOW | CONVERTTYPE.NOUDC);
            }
            
            return m_binder.mustConvert(argument, destinationType);
        }
        ...
        private EXPR BindExplicitConversion(ArgumentObject[] arguments, Type returnType, Dictionary<int, LocalVariableSymbol> dictionary)
        {
            ...
            return m_binder.mustCast(argument, destinationType);
        }
            ...
            Dictionary<int, LocalVariableSymbol> dictionary)
        {
            ...
            return m_binder.bindAssignment(lhs, rhs, bIsCompound);
        }
        #endregion
            ...
            Dictionary<int, LocalVariableSymbol> dictionary)
        {
            ...
            return m_exprFactory.CreateConstant(boolType, ConstValFactory.GetBool(result));
        }
        #endregion
