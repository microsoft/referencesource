...
namespace Microsoft.CSharp.RuntimeBinder
{
    internal class ExpressionTreeCallRewriter : ExprVisitorBase
    {
        ...
        protected override EXPR VisitSAVE(EXPRBINOP pExpr)
        {
            ...
            Expression parameter = ListOfParameters.ElementAt(currentParameterIndex++);
            DictionaryOfParameters.Add(call, parameter);
        ...
        protected override EXPR VisitCALL(EXPRCALL pExpr)
        {
            if (pExpr.PredefinedMethod != PREDEFMETH.PM_FIRST)
            {
                switch (pExpr.PredefinedMethod)
                {
                    ...
                    default:
                        Debug.Assert(false, "RuntimeBinderInternalCompilerException", "Invalid Predefined Method in VisitCALL");
        ...
        private ExpressionEXPR GenerateLambda(EXPRCALL pExpr)
        {
            ...
            //
            // TODO: What do we do with the initializer?
        ...
        private ExpressionEXPR GenerateCall(EXPRCALL pExpr)
        {
            ...
            if (m == null)
            {
                Debug.Assert(false, "RuntimeBinderInternalCompilerException", "How did we get a call that doesn't have a methodinfo?");
                throw Error.InternalCompilerError();
            }
        ...
        private ExpressionEXPR GenerateProperty(EXPRCALL pExpr)
        {
            ...
            if (p == null)
            {
                Debug.Assert(false, "RuntimeBinderInternalCompilerException", "How did we get a prop that doesn't have a propinfo?");
                throw Error.InternalCompilerError();
            }
...
#else
            // This is to fix Silverlight Bug #85557, in which we cannot call
            // Activator.CreateInstance using a type that is not accessible.
            // This will also work on the desktop and should be ported back
            // post-Dev10, when the branch opens up. See definition of GetObject
            // below as well.

            object v = GetObject(list.GetOptionalElement());
            Type t = list.GetOptionalNextListNode().asTYPEOF().SourceType.type.AssociatedSystemType;

            if (v == null)
            {
                return new ExpressionEXPR(Expression.Default(t));
            }

            return new ExpressionEXPR(Expression.Constant(v, t));
#endif
        }
        ...
        private ExpressionEXPR GenerateBinaryOperator(EXPRCALL pExpr)
        {
            ...
            switch (pExpr.PredefinedMethod)
            {
                ...
                default:
                    Debug.Assert(false, "RuntimeBinderInternalCompilerException", "Invalid Predefined Method in GenerateBinaryOperator");
                    throw Error.InternalCompilerError();
            }
        }
        ...
        private ExpressionEXPR GenerateUserDefinedBinaryOperator(EXPRCALL pExpr)
        {
            ...
            switch (pExpr.PredefinedMethod)
            {
                ...
                default:
                    Debug.Assert(false, "RuntimeBinderInternalCompilerException", "Invalid Predefined Method in GenerateUserDefinedBinaryOperator");
                    throw Error.InternalCompilerError();
            }
        }
        ...
        private ExpressionEXPR GenerateUnaryOperator(EXPRCALL pExpr)
        {
            ...
            switch (pm)
            {
                ...
                default:
                    Debug.Assert(false, "RuntimeBinderInternalCompilerException", "Invalid Predefined Method in GenerateUnaryOperator");
                    throw Error.InternalCompilerError();
            }
        }
        ...
        private ExpressionEXPR GenerateUserDefinedUnaryOperator(EXPRCALL pExpr)
        {
            ...
            switch (pm)
            {
                ...
                default:
                    Debug.Assert(false, "RuntimeBinderInternalCompilerException", "Invalid Predefined Method in GenerateUserDefinedUnaryOperator");
                    throw Error.InternalCompilerError();
            }
        }
        #endregion
        ...
        private Expression GetExpression(EXPR pExpr)
        {
            ...
            else
            {
                ...
                switch (pm)
                {
                    ...
                    default:
                        Debug.Assert(false, "RuntimeBinderInternalCompilerException", "Invalid Predefined Method in GetExpression");
                        throw Error.InternalCompilerError();
                }
            }
        }
        ...
        private object GetObject(EXPR pExpr)
        {
            ...
            else if (pExpr.isZEROINIT())
            {
                ...
                // Activator.CreateInstance using a type that is not accessible.
                // This will also work on the desktop and should be ported back
                // post-Dev10, when the branch opens up. See call to GetObject above
                // as well.
                return null;
#endif
            }

            Debug.Assert(false, "RuntimeBinderInternalCompilerException", "Invalid EXPR in GetObject");
            throw Error.InternalCompilerError();
        }
        ...
        private MethodInfo GetMethodInfoFromExpr(EXPRMETHODINFO methinfo)
        {
            ...
            TypeArray genericParams = m_typeManager.SubstTypeArray(methSym.Params, aggType, methSym.typeVars);
            CType genericReturn = m_typeManager.SubstType(methSym.RetType, aggType, methSym.typeVars);
            ...
            Debug.Assert(false, "RuntimeBinderInternalCompilerException", "Could not find matching method");
            throw Error.InternalCompilerError();
        }
        ...
        private ConstructorInfo GetConstructorInfoFromExpr(EXPRMETHODINFO methinfo)
        {
            ...
            TypeArray genericInstanceParams = m_typeManager.SubstTypeArray(methSym.Params, aggType);
            Type type = aggType.AssociatedSystemType;
            ...
            Debug.Assert(false, "RuntimeBinderInternalCompilerException", "Could not find matching constructor");
            throw Error.InternalCompilerError();
        }
        ...
        private PropertyInfo GetPropertyInfoFromExpr(EXPRPropertyInfo propinfo)
        {
            ...
            TypeArray genericInstanceParams = m_typeManager.SubstTypeArray(propSym.Params, aggType, null);
            CType genericInstanceReturn = m_typeManager.SubstType(propSym.RetType, aggType, null);
            ...
            Debug.Assert(false, "RuntimeBinderInternalCompilerException", "Could not find matching property");
            throw Error.InternalCompilerError();
        }
