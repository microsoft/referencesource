    ...
    internal static class BinderHelper
    {
                ...
                DynamicMetaObject onBindingError)
        {
            ...
            try
            {
                ...
                if (tempForIncrement != null)
                {
                    ...
                    DynamicMetaObject arg0 = args.First();
        ...
        internal static bool IsWindowsRuntimeObject(DynamicMetaObject obj)
        {
            if (obj != null && obj.RuntimeType != null)
            {
                ...
                while(curType != null)
                {
                    if (curType.Attributes.HasFlag(System.Reflection.TypeAttributes.WindowsRuntime))
                    {
                        // Found a WinRT COM object
                        return true;
                    }
                    if (curType.Attributes.HasFlag(System.Reflection.TypeAttributes.Import))
                    {
...
#if !SILVERLIGHT
            Debug.Assert(binding.Type.IsEquivalentTo(action.ReturnType) ||
                !binding.Type.IsValueType && !action.ReturnType.IsValueType && action.ReturnType.IsAssignableFrom(binding.Type));
#endif
            return binding;
        }
