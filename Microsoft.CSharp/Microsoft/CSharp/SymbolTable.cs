...
namespace Microsoft.CSharp.RuntimeBinder
{
    internal class SymbolTable
    {
        ...
        {
            Func<MethodBase, bool> isInvokableDelegate = null;

            MethodInfo isInvokable = typeof(MethodBase).GetMethod(
            ...
            if (isInvokable != null)
            {
                ...
                // AppDomains in a Windows Store app process are always Homogenous and fully trusted
                if (currentDomain.IsHomogenous && currentDomain.IsFullyTrusted)
                {
                    try
                    {
                        isInvokableDelegate = (Func<MethodBase, bool>)isInvokable.CreateDelegate(typeof(Func<MethodBase, bool>));
                    }
                    catch (System.Security.SecurityException)
                    {
                        // Microsoft.CSharp.dll is fully transparent so we cannot do a security assert here.
                        // CreateDelegate will do a MemberAccess demand here since MethodBase.IsDynamicallyInvokable is internal.
                        // That will fail with a SecurityException in partial trust in which case we will know that we are not in
                        // a Windows Store app process. We can ignore that exception because the Reflection Safe feature
                        // is only enabled in a Windows Store app process. In classic scenarios IsDynamicallyInvokable will alwasy be true anyway.
            ...
            Type callingType)
        {
            ...
            {
                // TODO: What about named indexers?
            ...
            if (m_namesLoadedForEachType.Contains(key))
            {
                return;
            }

            // Add the names.
            IEnumerable<MemberInfo> members = AddNamesOnType(key, BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Static);
            ...
            bool requireInvocable)
        {
            ...
            if (type.IsArrayType())
            {
                type = m_semanticChecker.GetSymbolLoader().GetReqPredefType(PredefinedType.PT_ARRAY);
            }
            if (type.IsNullableType())
            {
                type = type.AsNullableType().GetAts(m_semanticChecker.GetSymbolLoader().GetErrorContext());
            }
        ...
        {
            Debug.Assert(!m_namesLoadedForEachType.Contains(key));
        ...
        private IEnumerable<MemberInfo> AddNamesInInheritanceHierarchy(string name, BindingFlags flags, List<Type> inheritance)
        {
            ...
            foreach (Type t in inheritance)
            {
                ...
                IEnumerable<MemberInfo> members = from member in type.GetMembers(flags)
                                                  where member.Name == name && member.DeclaringType == type
                                                  select member;
                IEnumerable<MemberInfo> events = from member in type.GetMembers(flags)
                ...
                m_namesLoadedForEachType.Add(key);
            }
        ...
        private List<Type> CreateInheritanceHierarchyList(Type type)
        {
            ...
            {
                TypeArray collectioniFaces = ctype.AsAggregateType().GetWinRTCollectionIfacesAll(m_semanticChecker.GetSymbolLoader());
        ...
        private Name GetName(string p)
        {
            ...
            return SymbolTable.GetName(p, m_nameManager);
        }
        ...
        private Name GetName(Type type)
        {
            ...
            return SymbolTable.GetName(name, m_nameManager);
        }
            ...
            if (method.IsGenericMethod)
            {
                ...
                for (int i = 0; i < genericArguments.Length; i++)
                {
                    ...
                    ctypes[i].AsTypeParameterType().GetTypeParameterSymbol().SetBounds(
                        m_bsymmgr.AllocParams(
                        GetCTypeArrayFromTypes(t.GetGenericParameterConstraints())));
                }
                return m_bsymmgr.AllocParams(ctypes.Length, ctypes);
            }
        ...
        private TypeArray GetAggregateTypeParameters(Type type, AggregateSymbol agg)
        {
            if (type.IsGenericType)
            {
                ...
                return m_bsymmgr.AllocParams(ctypes.Count, ctypes.ToArray());
            }
            return BSYMMGR.EmptyTypeArray();
        }
        ...
        private TypeParameterType LoadClassTypeParameter(AggregateSymbol parent, Type t)
        {
            ...
            {
                for (TypeParameterSymbol typeParam = m_bsymmgr.LookupAggMember(
        ...
        // generalize these occurrences for reference equality. We don't need to because
        // the bug this solves, 846409, only suffers problems on class type parameters.
        //
                ...
                bool bIsAggregate)
        {
            ...
            if (bIsAggregate)
            {
                typeParam = m_symFactory.CreateClassTypeParameter(
                    GetName(t),
                    agg,
            ...
            {
                typeParam = m_symFactory.CreateMethodTypeParameter(
                    GetName(t),
                    meth,
                    t.GenericParameterPosition,
                    t.GenericParameterPosition);
            ...
            TypeParameterType typeParamType = m_typeManager.GetTypeParameter(typeParam);
        ...
        private CType LoadSymbolsFromType(Type originalType)
        {
            ...
            NamespaceOrAggregateSymbol current = m_rootNamespace;
            ...
            for (int i = 0; i < declarationChain.Count; i++)
            {
                ...
                if (o is Type)
                {
                    ...
                    Name name = null;
                    name = GetName(t);
                    next = m_symbolTable.LookupSym(name, current, symbmask_t.MASK_AggregateSymbol).AsAggregateSymbol();
            ...
            if (bIsByRef)
            {
                ret = m_typeManager.GetParameterModifier(ret, false);
            }
            return ret;
        }
        ...
        private CType GetConstructedType(Type type, AggregateSymbol agg)
        {
            ...
            if (type.IsGenericType)
            {
                ...
                TypeArray typeArray = m_bsymmgr.AllocParams(types.ToArray());
                AggregateType aggType = m_typeManager.GetAggregate(agg, typeArray);
                return aggType;
            }
        ...
        private CType ProcessSpecialTypeInChain(NamespaceOrAggregateSymbol parent, Type t)
        {
            ...
            else if (t.IsArray)
            {
                ...
                ctype = m_typeManager.GetArray(GetCTypeFromType(t.GetElementType()), t.GetArrayRank());
                return ctype;
            }
            else if (t.IsPointer)
            {
                // Now we return the pointer type that we want.
                ctype = m_typeManager.GetPointer(GetCTypeFromType(t.GetElementType()));
                return ctype;
            }
            else if (t.IsNullableType())
            {
                ...
                if (t.GetGenericArguments()[0].DeclaringType == t)
                {
                    ...
                    AggregateSymbol agg = m_symbolTable.LookupSym(
                ...
                ctype = m_typeManager.GetNullable(GetCTypeFromType(t.GetGenericArguments()[0]));
                return ctype;
            }
            return null;
        }
        ...
        private static List<object> BuildDeclarationChain(Type callingType)
        {
            ...
            for (Type t = callingType; t != null; t = t.DeclaringType)
            {
                ...
                if (t.IsGenericParameter && t.DeclaringMethod != null)
                {
                    ...
                    bool bAdded = false;
                    foreach (MethodInfo methinfo in from m in t.DeclaringType.GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Static)
        ...
        private NamespaceSymbol AddNamespaceToSymbolTable(NamespaceOrAggregateSymbol parent, string sz)
        {
            ...
            NamespaceSymbol ns = m_symbolTable.LookupSym(name, parent, symbmask_t.MASK_NamespaceSymbol).AsNamespaceSymbol();
            if (ns == null)
            {
                ns = m_symFactory.CreateNamespace(name, parent as NamespaceSymbol);
            }
            ns.AddAid(KAID.kaidGlobal);
            ns.AddAid(KAID.kaidThisAssembly);
            ns.AddAid(m_infile.GetAssemblyID());
        ...
        private AggregateSymbol AddAggregateToSymbolTable(
            ...
            Type type)
        {
            AggregateSymbol agg = m_symFactory.CreateAggregate(GetName(type), parent, m_infile, m_typeManager);
            ...
            agg.SetTypeManager(m_typeManager);
            agg.SetFirstUDConversion(null);
            SetInterfacesOnAggregate(agg, type);
            agg.SetHasPubNoArgCtor(type.GetConstructors().Any(c => c.GetParameters().Length == 0));
        ...
        {
            FieldSymbol field = m_symbolTable.LookupSym(
            ...
            field = m_symFactory.CreateMemberVar(GetName(fieldInfo.Name), aggregate, null, 0);
        ...
        {
            EventSymbol ev = m_symbolTable.LookupSym(
            ...
            ev = m_symFactory.CreateEvent(GetName(eventInfo.Name), aggregate, null);
            ...
            {
                ev.methAdd = AddMethodToSymbolTable(eventInfo.GetAddMethod(true), aggregate, MethodKindEnum.EventAccessor);
                ev.methAdd.SetEvent(ev);
                ev.isOverride = ev.methAdd.IsOverride();

                access = ev.methAdd.GetAccess();
            }
            if (eventInfo.GetRemoveMethod(true) != null)
            {
                ev.methRemove = AddMethodToSymbolTable(eventInfo.GetRemoveMethod(true), aggregate, MethodKindEnum.EventAccessor);
            ...
            if (ev.methAdd.RetType.AssociatedSystemType == typeof(EventRegistrationToken) &&
                ev.methRemove.Params.Item(0).AssociatedSystemType == typeof(EventRegistrationToken))
            {
            ...
            Type evtRegTokenTable = typeof(EventRegistrationTokenTable<>).MakeGenericType(ev.type.AssociatedSystemType);
            if (addedField != null && addedField.GetType() != null &&
                (addedField.GetType() == ev.type ||
                 addedField.GetType().AssociatedSystemType == evtRegTokenTable))
            {
        ...
        internal void AddPredefinedPropertyToSymbolTable(AggregateSymbol type, Name property)
        {
            ...
            var props = from x in t.GetProperties()
        ...
        private PropertySymbol AddPropertyToSymbolTable(PropertyInfo property, AggregateSymbol aggregate)
        {
            ...
            PropertySymbol prop = m_symbolTable.LookupSym(
                name,
                aggregate,
            ...
            if (prop != null)
            {
                ...
                while (prop != null)
                {
                    ...
                    prevProp = prop;
                    prop = m_semanticChecker.SymbolLoader.LookupNextSym(prop, prop.parent, symbmask_t.MASK_PropertySymbol).AsPropertySymbol();
                }
            ...
            if (prop == null)
            {
                if (isIndexer)
                {
                    prop = m_semanticChecker.GetSymbolLoader().GetGlobalMiscSymFactory().CreateIndexer(name, aggregate, GetName(property.Name), null);
                    prop.Params = CreateParameterArray(null, property.GetIndexParameters());
                }
                else
                {
                    prop = m_symFactory.CreateProperty(GetName(property.Name), aggregate, null);
            ...
            if (property.GetAccessors(true) != null)
            {
                MethodInfo accessor = property.GetAccessors(true)[0]; // Must have at least one.
            ...
            // Get and set.
            MethodInfo methGet = property.GetGetMethod(true);
            MethodInfo methSet = property.GetSetMethod(true);
            ACCESS access = ACCESS.ACC_PRIVATE;
            if (methGet != null)
            {
        ...
        internal void AddPredefinedMethodToSymbolTable(AggregateSymbol type, Name methodName)
        {
            ...
            {
                var ctors = from m in t.GetConstructors()
        ...
        private static bool IsMethodDynamicallyInvokable(MethodBase method)
        {
            ...
            return s_IsInvokableDelegate == null || 
        ...
        private MethodSymbol AddMethodToSymbolTable(MemberInfo member, AggregateSymbol callingAggregate, MethodKindEnum kind)
        {
            ...
            if (method != null)
            {
                if (method.IsPublic && IsMethodDynamicallyInvokable(method))
                {
                    access = ACCESS.ACC_PUBLIC;
                }
                else if (method.IsPrivate || (method.IsPublic && !IsMethodDynamicallyInvokable(method)))
                {
                    access = ACCESS.ACC_PRIVATE;
                }
            ...
            else
            {
                ...
                if (ctor.IsPublic && IsMethodDynamicallyInvokable(ctor))
                {
                    access = ACCESS.ACC_PUBLIC;
                }
                else if (ctor.IsPrivate || (ctor.IsPublic && !IsMethodDynamicallyInvokable(ctor)))
                {
                    access = ACCESS.ACC_PRIVATE;
                }
        ...
        private void SetParameterDataForMethProp(MethodOrPropertySymbol methProp, ParameterInfo[] parameters)
        {
            if (parameters.Length > 0)
            {
                ...
                object[] attributes = parameters[parameters.Length - 1].GetCustomAttributes(false);
                if (attributes != null)
                {
        ...
        private void SetParameterAttributes(MethodOrPropertySymbol methProp, ParameterInfo[] parameters, int i)
        {
            ...
            {
                if ((attrs = parameters[i].GetCustomAttributes(typeof(MarshalAsAttribute), false)) != null
                    && attrs.Length > 0)
                {
...
#if !SILVERLIGHT
            // Get IUnknownConstantAttribute
            if ((attrs = parameters[i].GetCustomAttributes(typeof(IUnknownConstantAttribute), false)) != null
                && attrs.Length > 0)
            {
...
#if !SILVERLIGHT
            // GetIDispatchConstantAttribute
            if ((attrs = parameters[i].GetCustomAttributes(typeof(IDispatchConstantAttribute), false)) != null
                && attrs.Length > 0)
            {
            ...
            if ((attrs = parameters[i].GetCustomAttributes(typeof(DateTimeConstantAttribute), false)) != null
                && attrs.Length > 0)
            {
                ...
                CType cvType = m_semanticChecker.GetSymbolLoader().GetReqPredefType(PredefinedType.PT_DATETIME);
                methProp.SetDefaultParameterValue(i, cvType, cv);
            }
            else if ((attrs = parameters[i].GetCustomAttributes(typeof(DecimalConstantAttribute), false)) != null
                && attrs.Length > 0)
            {
                // Get DecimalConstant
                ...
                CType cvType = m_semanticChecker.GetSymbolLoader().GetOptPredefType(PredefinedType.PT_DECIMAL);
                methProp.SetDefaultParameterValue(i, cvType, cv);
            }
                ...
                !parameters[i].ParameterType.IsByRef)
            {
                ...
                CType cvType = m_semanticChecker.GetSymbolLoader().GetReqPredefType(PredefinedType.PT_OBJECT);
                ...
                if (parameters[i].RawDefaultValue != null)
                {
                    ...
                    if (defType == typeof(System.Byte))
                    {
                        ...
                        cvType = m_semanticChecker.GetSymbolLoader().GetReqPredefType(PredefinedType.PT_BYTE);
                    }
                    else if (defType == typeof(System.Int16))
                    {
                        cv = factory.Create((System.Int16)defValue);
                        cvType = m_semanticChecker.GetSymbolLoader().GetReqPredefType(PredefinedType.PT_SHORT);
                    }
                    else if (defType == typeof(System.Int32))
                    {
                        cv = factory.Create((System.Int32)defValue);
                        cvType = m_semanticChecker.GetSymbolLoader().GetReqPredefType(PredefinedType.PT_INT);
                    }
                    else if (defType == typeof(System.Int64))
                    {
                        cv = factory.Create((System.Int64)defValue);
                        cvType = m_semanticChecker.GetSymbolLoader().GetReqPredefType(PredefinedType.PT_LONG);
                    }
                    else if (defType == typeof(System.Single))
                    {
                        cv = factory.Create((System.Single)defValue);
                        cvType = m_semanticChecker.GetSymbolLoader().GetReqPredefType(PredefinedType.PT_FLOAT);
                    }
                    else if (defType == typeof(System.Double))
                    {
                        cv = factory.Create((System.Double)defValue);
                        cvType = m_semanticChecker.GetSymbolLoader().GetReqPredefType(PredefinedType.PT_DOUBLE);
                    }
                    else if (defType == typeof(System.Decimal))
                    {
                        cv = factory.Create((System.Decimal)defValue);
                        cvType = m_semanticChecker.GetSymbolLoader().GetReqPredefType(PredefinedType.PT_DECIMAL);
                    }
                    else if (defType == typeof(System.Char))
                    {
                        cv = factory.Create((System.Char)defValue);
                        cvType = m_semanticChecker.GetSymbolLoader().GetReqPredefType(PredefinedType.PT_CHAR);
                    }
                    else if (defType == typeof(System.Boolean))
                    {
                        cv = factory.Create((System.Boolean)defValue);
                        cvType = m_semanticChecker.GetSymbolLoader().GetReqPredefType(PredefinedType.PT_BOOL);
                    }
                    else if (defType == typeof(System.SByte))
                    {
                        cv = factory.Create((System.SByte)defValue);
                        cvType = m_semanticChecker.GetSymbolLoader().GetReqPredefType(PredefinedType.PT_SBYTE);
                    }
                    else if (defType == typeof(System.UInt16))
                    {
                        cv = factory.Create((System.UInt16)defValue);
                        cvType = m_semanticChecker.GetSymbolLoader().GetReqPredefType(PredefinedType.PT_USHORT);
                    }
                    else if (defType == typeof(System.UInt32))
                    {
                        cv = factory.Create((System.UInt32)defValue);
                        cvType = m_semanticChecker.GetSymbolLoader().GetReqPredefType(PredefinedType.PT_UINT);
                    }
                    else if (defType == typeof(System.UInt64))
                    {
                        cv = factory.Create((System.UInt64)defValue);
                        cvType = m_semanticChecker.GetSymbolLoader().GetReqPredefType(PredefinedType.PT_ULONG);
                    }
                    else if (defType == typeof(System.String))
                    {
                        cv = factory.Create((System.String)defValue);
                        cvType = m_semanticChecker.GetSymbolLoader().GetReqPredefType(PredefinedType.PT_STRING);
                    }
        ...
        {
            MethodSymbol meth = m_bsymmgr.LookupAggMember(GetName(method.Name), callingAggregate, symbmask_t.MASK_MethodSymbol).AsMethodSymbol();
            while (meth != null)
            {
        ...
        private TypeArray CreateParameterArray(MemberInfo associatedInfo, ParameterInfo[] parameters)
        {
            ...
            {
                types.Add(m_typeManager.GetArgListType());
            }

            return m_bsymmgr.AllocParams(types.Count, types.ToArray());
        }
        ...
        private CType GetTypeOfParameter(ParameterInfo p, MemberInfo m)
        {
            ...
            if (ctype.IsParameterModifierType() && p.IsOut && ((p.Attributes & ParameterAttributes.In) == 0))
#else
            if (ctype.IsParameterModifierType() && p.IsOut && !p.IsIn)
#endif
            {
                CType parameterType = ctype.AsParameterModifierType().GetParameterType();
                ctype = m_typeManager.GetParameterModifier(parameterType, true);
            }
        ...
        private MethodSymbol FindMethodFromMemberInfo(MemberInfo baseMemberInfo)
        {
            ...
            MethodSymbol meth = m_semanticChecker.SymbolLoader.LookupAggMember(
            ...
            for (;
                    ...
                    meth = m_semanticChecker.SymbolLoader.LookupNextSym(meth, aggregate, symbmask_t.MASK_MethodSymbol).AsMethodSymbol()) ;
        ...
        private void AddConversionsForOneType(Type type)
        {
            ...
            IEnumerable<MethodInfo> conversions = from conversion in type.GetMethods(
                                  BindingFlags.Public | BindingFlags.Static)
                                                  where (conversion.Name == SpecialNames.ImplicitConversion || conversion.Name == SpecialNames.ExplicitConversion)
    ...
    internal static class RuntimeBinderExtensions
    {
...
#if SILVERLIGHT
        // In Silverlight, Type.IsEquivalentTo is not public. And worse, Type is a MemberInfo,
        // so we need to establish the correct behavior for Type.IsEquivalentTo in contrast
        // to the MemberInfo.IsEquivalentTo extension method.

        internal static bool IsEquivalentTo(this Type t1, Type t2)
        {
            return t1 == t2;
        }
#endif
        ...
        // No-PIA types. It is used above, by the symbol table, to determine whether
        ...
        internal static bool IsEquivalentTo(this MemberInfo mi1, MemberInfo mi2)
        {
            ...
            if (mi1 is MethodInfo && mi2 is MethodInfo)
            {
                ...
                // Note: we don't allow generic methods here because No-PIA methods
                // are never generic, and allowing for them would unnecc. complicate
                // the tests.
                ...
                return method1 != method2 
                    ...
                    && method1.DeclaringType.IsEquivalentTo(method2.DeclaringType)
                    && method1.ReturnType.IsEquivalentTo(method2.ReturnType)
                    && (pis1 = method1.GetParameters()).Length == (pis2 = method2.GetParameters()).Length
                    && Enumerable.Zip(pis1, pis2, (pi1, pi2) => pi1.IsEquivalentTo(pi2)).All(x => x);
            }
            ...
            if (mi1 is ConstructorInfo && mi2 is ConstructorInfo)
            {
                ...
                return ctor1 != ctor2 
                    && ctor1.DeclaringType.IsEquivalentTo(ctor2.DeclaringType)
            ...
            if (mi1 is PropertyInfo && mi2 is PropertyInfo)
            {
                ...
                return prop1 != prop2 
                    ...
                    && prop1.DeclaringType.IsEquivalentTo(prop2.DeclaringType)
                    && prop1.PropertyType.IsEquivalentTo(prop2.PropertyType)
        ...
        internal static bool IsGenericallyEqual(this Type t1, Type t2)
        {
            ...
            {
                Type t1def = t1.GetGenericTypeDefinition();
                Type t2def = t2.GetGenericTypeDefinition();
