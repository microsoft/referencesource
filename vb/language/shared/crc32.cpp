...
//
//  The faster table version.  Use the init routine below with the reversed CRC polynomial 0x d0ab 0f00 02c0 0600
//  to create it
//  Note: the actual poly is a reversed version with the leading '1' implied
//
//  poly = x^32   + x^26 + x^23 + x^22 + x^16 + x^12 + x^11 + x^10 + x^8 + x^7 + x^5 + x^4 + x^2 + x + 1
//         (1)00000 100    1      100000 1000   1      1      10     1     10    1     10    1     1   1 (0x104C11DB7)
//
//  reverse poly = x^32 + x^31 + x^30 + x^29 + x^27 + x^26 + x^24 + x^23 + x^21 + x^20 + x^19 + x^15 + x^9 + x^8 + x^5
//                 (1)    1      1      10     1      10     1      10     1      1      1000   100000 1     100   100000 (0x1EDB88320)
//
//-------------------------------------------------------------------------------------------------
...
//#define USE_CRC_TABLE
...
#ifdef USE_CRC_TABLE
    static const DWORD CRC32_LOOKUP_TABLE [256] = {
    ...
    // Delete this code and you cannot regenerate the table!!! (you may want to switch polys for example)

    static const DWORD CRC32_poly = 0xEDB88320;
    static bool CRC32_LOOKUP_TABLE_INITIALIZED = false;

    static DWORD CRC32_LOOKUP_TABLE [256];
    ...
    void initCRC32table(
        DWORD poly,
        _Inout_count_(256) DWORD CrcXorTable[256])
    {
        for (unsigned i = 0; i < 256; ++i) {
            DWORD entry = calc_entry( i, poly );
            CrcXorTable[i] = entry;
    ...
    void inittable()
    {
        if( !CRC32_LOOKUP_TABLE_INITIALIZED ) {
            initCRC32table( CRC32_poly, CRC32_LOOKUP_TABLE );
            CRC32_LOOKUP_TABLE_INITIALIZED = true;
        }
    }
#endif
...
DWORD NewCRC32(
    ...
    BYTE bNextByte)
{
#pragma prefast(suppress: 26010, "There is a BYTE cast which Prefast is ignoring")
    return CRC32_LOOKUP_TABLE[ (BYTE)(crc ^ bNextByte) ] ^ (crc >> 8);
}
