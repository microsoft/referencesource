...
//  Sys.WebForms.Menu
//
//    A wrapper class around a menu/submenu (whose element is an HTML UL element)
//    It is also called for the initial menu (something we should clean up), so it has
//    code in it when recognizes the difference between "a new menu" and "a new submenu"
...
Sys.WebForms.Menu = function(options) {
    ...
    if (this.element.tagName === 'DIV') {
        ...
        // set float on the main UL so it contains the floating LI's, otherwise
        // it collapses on itself, which messes up the background color and border, if any.
    ...
    for (var i = 0; i < count; i++) {
        ...
        // set up siblings for easy navigation
...
Sys.WebForms.Menu.prototype = {
    ...
    collapse: function() {
        //collapse this and all child menus
...
// -------------------------------------------------------------------------------------
//  Sys.WebForms.MenuItem
//    A wrapper class around a menu item (whose element is an HTML LI element)
...
Sys.WebForms.Menu._domHelper = {
    ...
    firstChild: function(element) {
        ...
        if (child && child.nodeType !== 1) {   // only element nodes
    ...
    nextSibling: function(element) {
        ...
        while (sibling) {
            if (sibling.nodeType === 1) {   // only element nodes
                return sibling;
            }
...
//    Maps elements to their wrapper objects so they can be matched later. Cannot
//    rely on HTML IDs, because not every object will have them, and we don't want
//    to hang the wrappers off the elements as expandos because of memory issues.
...
Sys.WebForms.Menu._elementObjectMapper = {
    ...
    map: function(element, theObject) {
        ...
        var objectId = element[this._mappingIdName] || element.id || '%' + (++this._computedId); // '%' makes sure it's a string and ensures no id conflict.
...
// -------------------------------------------------------------------------------------
//  Sys.WebForms.Menu._keyboardMapping
...
// -------------------------------------------------------------------------------------
//  Sys.WebForms._MenuContainer
//    A wrapper class around the outer DIV which surrounds a menu
...
Sys.WebForms._MenuContainer = function(options) {
    ...
    // Float the DIV, and create a clear:left immediate after
    // This lets the menu be only as wide as it needs to be, instead of 100% width
