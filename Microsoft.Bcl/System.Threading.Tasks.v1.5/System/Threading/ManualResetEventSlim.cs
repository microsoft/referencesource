...
namespace System.Threading
{
    ...
    internal class ManualResetEventSlim : IDisposable
    {
        ...
        public ManualResetEventSlim(bool initialState, int spinCount)
        {
            ...
            if (spinCount > SpinCountState_MaxValue)
            {
                throw new ArgumentOutOfRangeException(
                    "spinCount",
                    String.Format(Strings.ManualResetEventSlim_ctor_SpinCountOutOfRange,SpinCountState_MaxValue));
            }
            ...
            if (!IsSet)
            {
                ...
                for (int i = 0; i < SpinCount; i++)
                {
                    ...
                    else if (i < HOW_MANY_SPIN_BEFORE_YIELD)
                    {
                        ...
                        {
                            ThreadLightup.Current.SpinWait(PlatformHelper.ProcessorCount * (4 << i));
                        }
                    }
        ...
        {
            if ((m_combinedState & Dispose_BitMask) != 0)
                throw new ObjectDisposedException(Strings.ManualResetEventSlim_Disposed);
        }
